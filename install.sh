...
# Repo location, derived from this script (works wherever you cloned it)
...
# ------------------------------------------
clear
echo -e "\e[33m🔑 Requesting administrative privileges...\e[0m"
sudo -v
while true; do
  sudo -n -v
  sleep 60
...
# ==========================================
# 0. BOOTSTRAP — must run before any gum call
# ==========================================
if [ ! -d "$REPO/.config" ]; then
  echo -e "\e[31m[!] Error: $REPO/.config not found. Is this the dotfiles repo?\e[0m"
  exit 1
fi

echo -e "\e[34mInstalling base tooling...\e[0m"
...
if ! command -v gum &>/dev/null; then
  ...
  if ! sudo pacman -S --needed --noconfirm gum; then
    echo -e "\e[31m[!] Failed to install 'gum'. Check your internet connection or pacman lock.\e[0m"
    exit 1
  fi
fi
...
if [[ "$WANT_NVIM" == "yes" ]]; then
  ...
  # Config ships as a submodule at .config/nvim and is deployed by stow below
  gum spin --spinner dot --spinner.foreground "$COLOR_ACCENT" --title " Fetching RAINZ configuration..." \
    -- git -C "$REPO" submodule update --init --recursive .config/nvim >/dev/null 2>&1
...
# Configs live in $REPO/.config/<app>, so stow from there into ~/.config
safe_stow() {
  local app=$1
  if [ ! -d "$REPO/.config/$app" ]; then
    print_info "Skipping $app (not in repo)."
    return 0
  fi
  # || true prevents stow errors from triggering the set -e crash trap
  gum spin --spinner mini --spinner.foreground "$COLOR_SECONDARY" --title " Stowing $app..." \
    -- stow -d "$REPO/.config" -t "$HOME/.config" --adopt "$app" >/dev/null 2>&1 || true
}

mkdir -p "$HOME/.config"

if [[ "$INSTALL_FONTS" == "yes" ]]; then safe_stow fish; fi
if [[ "$INSTALL_HYPR" == "yes" ]]; then safe_stow rofi; safe_stow hypr; safe_stow tmux; fi
if [[ "$INSTALL_NOCT" == "yes" ]]; then safe_stow noctalia; fi
if [[ "$WANT_NVIM" == "yes" ]]; then safe_stow nvim; fi
...
if [[ $EXTRAS == *"Fetch Tools"* ]]; then safe_stow fastfetch; safe_stow batfetch; fi

git -C "$REPO" restore .config >/dev/null 2>&1 || true
